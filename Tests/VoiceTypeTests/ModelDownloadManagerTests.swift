...
/// Task 1 RED: Tests for ModelDownloadManager, TranscriptionError, and filler word removal.
///
/// These tests MUST fail on first run — the types under test don't exist yet.

@Suite("ModelDownloadManager — Model State & Error Types")
struct ModelDownloadManagerTests {

    // MARK: - ModelState transitions

    @Test("ModelState starts as notLoaded")
    ...
    func transcriptionErrorCases() {
        // Verify all cases exist by constructing them
