...
**Confidence:** HIGH (verified against GitHub READMEs and official docs for all core recommendations)
...
**Why native Swift over alternatives:**
...
**Audio pipeline:** `AVAudioEngine input node → tap install → Float32 PCM buffer → WhisperKit.transcribe(audioArray:)`
...
**Recommended approach:** Use CGEvent tap for the dictation hotkey (push-to-talk, hold-to-record). The CGEvent tap captures both key-down and key-up events system-wide, which is essential for the "hold to talk, release to stop" interaction pattern. Carbon's `RegisterEventHotKey` (used by soffes/HotKey) only fires on key-down, making hold-to-talk harder to implement.
...
**Required entitlements:** `com.apple.security.automation.apple-events` and accessibility permission (user must grant in System Settings → Privacy → Accessibility).
...
**Prompt engineering approach:** Send context (surrounding text from accessibility API) + correction instruction (user's spoken command transcribed by Whisper) → LLM returns corrected text → replace in target app via accessibility API.
...
**If Anthropic integration is needed:**
...
---
*Stack research for: macOS voice dictation with AI correction*
*Researched: 2026-07-30*
